...
หากต้องการเพิ่มหมวดหมู่ บอร์ดเกมตัวอย่าง และบัญชีผู้ดูแลครั้งแรก ให้ตั้ง `ADMIN_PASSWORD` เป็นรหัสผ่านที่ปลอดภัยและเปลี่ยน `RUN_SEEDERS=true` แล้ว deploy หนึ่งครั้ง Seeder ตรวจข้อมูลเดิมก่อนเพิ่มจึงไม่สร้างรายการซ้ำ และจะซิงก์ชื่อ รหัสผ่าน และสถานะของบัญชี `ADMIN_USERNAME` ตาม Variables หลังสำเร็จให้เปลี่ยน `RUN_SEEDERS=false`
